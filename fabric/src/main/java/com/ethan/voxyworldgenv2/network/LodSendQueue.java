...
public final class LodSendQueue {
    ...
    public boolean enqueue(ServerPlayer player, ResourceKey<Level> dimension, ChunkPos pos,
                           int minY, List<PendingSection> sections) {
        if (!running.get() || sections.isEmpty()) return false;
        if (queue.offer(new Job(player, dimension, pos, minY, sections))) {
            return true;
        }
    ...
    private void process(Job job) {
        ...
        for (PendingSection ps : job.sections()) {
            byte[] states = serialiseStates(ps.states());
    ...
    // On 26.2 the palette codec is registry-free -- PalettedContainerRO.write takes a plain
    // FriendlyByteBuf -- so no RegistryAccess is threaded through the pipeline.
    private byte[] serialiseStates(PalettedContainer<BlockState> states) {
