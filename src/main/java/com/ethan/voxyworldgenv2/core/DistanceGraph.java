...
public class DistanceGraph {
    ...
    private void recursiveMark(Node node, int bx, int bz, int bit) {
        ...
        if (node.level == 1) {
            ...
            // that atomic. Two concurrent marks on the same batch (reachable via
            // TellusIntegration.enqueueGenerate, which completes on its own pool rather than
            // through server.execute) lose a bit, and nothing else ever re-marks a chunk already in
            // completedChunks, so the loss is permanent: a fully generated batch stuck below
            // 0xFFFF, never full, re-offered by findWork forever. The node monitor was already
            // being taken two lines down, so widening it costs nothing.
            synchronized (node) {
                if ((node.fullMask & (1L << idx)) != 0) return;
