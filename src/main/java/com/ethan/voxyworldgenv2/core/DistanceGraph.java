...
public class DistanceGraph {
    ...
    private void recursiveMark(Node node, int bx, int bz, int bit) {
        ...
        if (node.level == 1) {
            ...
            // generated batch stuck below 0xFFFF, never full, re-offered by findWork forever. The
            // node monitor was already being taken two lines down for the full case, so widening it
            // costs nothing.
            synchronized (node) {
                if ((node.fullMask & (1L << idx)) != 0) return;
